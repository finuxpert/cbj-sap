...
def add_case_parsed_result(case_id: str, payload: ParsedResultCreate) -> dict[str, Any]:
    """Save one parsed result into a case while keeping case identity stable.

    A case represents one incident identity. RCA tools may append parsed results,
    classify case_stage/severity, and enrich SID/environment when missing, but
    they must not overwrite case-level summary, anomaly, suspect, title, or label.
    Tool findings remain in parsed_results and normalized_rca.
    """
