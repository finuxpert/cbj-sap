...
export default function CaseCard({ caseItem, onArchive, archiving = false, selected = false, onSelect }) {
  ...
  const title = item.title || item.case_no || 'Untitled SPHERE Case'
  const caseId = item.id || item.case_no || ''
  const summary = item.summary || 'Belum ada management summary. Upload dan parse evidence untuk generate SPHERE summary.'
