...
const statusColor = (value) => (/RED|CRIT/i.test(value) ? C.red : /YELLOW|WARN/i.test(value) ? C.yellow : C.green)
...
function addToc(pdf, page, y, toc) {
  pdf.addPage('a4', 'portrait')
  Object.assign(page, pageOf(pdf))
  y.value = 16
  write(pdf, page, y, 'Report Index', 15, 'bold', C.teal)
  write(pdf, page, y, 'Auto-generated page index for faster review and management sign-off.', 9, 'normal', C.muted)
  y.value += 4
  toc.forEach((item, index) => {
    ensure(pdf, page, y, 10)
    pdf.setDrawColor(...C.border)
    pdf.line(page.m, y.value + 2, page.w - page.m, y.value + 2)
    pdf.setTextColor(...C.ink)
    pdf.setFont('helvetica', 'bold')
    pdf.setFontSize(9)
    pdf.text(`${index + 1}. ${item.title}`, page.m, y.value)
    pdf.setTextColor(...C.teal)
    pdf.text(`Page ${item.page}`, page.w - page.m - 24, y.value)
    y.value += 9
  })
}

function recommendationMatrix(pdf, page, y, report) {
  ensure(pdf, page, y, 70)
  write(pdf, page, y, 'RCA Recommendation Matrix', 12, 'bold', C.teal)
  const rows = [
    ['Basis', `Check SM50/SM66 PID ${report.top.pid}`, 'Now', report.severity === 'CRITICAL' ? 'High' : 'Medium'],
    ['Job Owner', `Review SM37 job ${report.top.job}`, 'Next', report.top.job !== '-' ? 'High' : 'Medium'],
    ['Infrastructure', `Validate RSS ${report.metrics.maxRss} on ${report.top.host}`, 'Now', numberOf(report.metrics.maxRss) >= 32 ? 'High' : 'Low'],
    ['SAP Logs', 'Correlate ST22 / SM21 in same time window', 'Next', 'Medium'],
    ['Incident Mgmt', 'Attach source WP-SCOUT evidence and this PDF', 'Closeout', 'Medium'],
  ]
  const widths = [28, page.w - page.m * 2 - 92, 24, 40]
  let x = page.m
  ...
  pdf.setFont('helvetica', 'bold')
  pdf.setFontSize(7)
  ;['OWNER', 'ACTION', 'WHEN', 'PRIORITY'].forEach((header, index) => {
  ...
  rows.forEach((row, index) => {
    ...
    row.forEach((value, col) => {
      const color = col === 3 ? statusColor(value === 'High' ? 'RED' : value === 'Medium' ? 'YELLOW' : 'GREEN') : C.ink
      pdf.setTextColor(...color)
      pdf.setFont('helvetica', col === 0 || col === 3 ? 'bold' : 'normal')
      pdf.setFontSize(7.2)
      pdf.text(pdf.splitTextToSize(compact(value, 76), widths[col] - 2).slice(0, 1), x + 1, y.value + 6.5)
      x += widths[col]
    })
    y.value += 10
  })
...
function severityAppendix(pdf, y, report) {
  ...
  recommendationMatrix(pdf, page, y, report)
}
...
export async function exportWpScoutVisualPdf() {
  ...
  toc.push({ title: 'Executive RCA Cover', page: pdf.getNumberOfPages() })
  cover(pdf, page, y, report, chartImages.length)
  toc.push({ title: 'Report Index', page: pdf.getNumberOfPages() + 1 })
  addToc(pdf, page, y, toc)
  toc.push({ title: 'Landscape Visual Analytics', page: pdf.getNumberOfPages() + 1 })
  landscapeAnalytics(pdf, y, chartImages, report)
  toc.push({ title: 'Portrait Chart Evidence', page: pdf.getNumberOfPages() + 1 })
  portraitCharts(pdf, y, chartImages, report)
  toc.push({ title: 'Severity-Split Evidence Appendix', page: pdf.getNumberOfPages() + 1 })
  severityAppendix(pdf, y, report)
  footer(pdf, report)

  pdf.save(`sap-wpscout-enterprise-v4-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.pdf`)
}
