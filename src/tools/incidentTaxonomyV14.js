...
function timingForError(row = {}, code = '') {
  return (row.errorTimings || []).find((item) => String(item.error || '') === String(code || '')) || { error: code, state: 'NONE', deltaMinutes: null, firstTime: '' }
}
...
export function errorTaxonomyForRow(row = {}) {
  ...
  const precursor = classified.filter((item) => item.causalClass === 'CAUSAL_CAPABLE' && ['NEW_BEFORE_TARGET', 'NEW_AT_TARGET'].includes(item.timing?.state)).sort((a, b) => Math.abs(a.timing?.deltaMinutes ?? 9999) - Math.abs(b.timing?.deltaMinutes ?? 9999))[0] || null
...
function hostSignal(anchor = {}) {
  ...
  const cpu = metric(row.cpuPct); const ram = metric(row.memoryPct); const load = metric(row.resourceLoadRatio); const swapIn = metric(row.swapIn)
  ...
  const psiCpuSome = hostEnhancedUsable ? metric(row.psiCpuSome10) : null; const psiCpuFull = hostEnhancedUsable ? metric(row.psiCpuFull10) : null
  const psiMemSome = hostEnhancedUsable ? metric(row.psiMemorySome10) : null; const psiMemFull = hostEnhancedUsable ? metric(row.psiMemoryFull10) : null
  const psiIoSome = hostEnhancedUsable ? metric(row.psiIoSome10) : null; const psiIoFull = hostEnhancedUsable ? metric(row.psiIoFull10) : null
  return {
    cpu, ram, load, swapIn, iowait, psiCpuSome, psiCpuFull, psiMemSome, psiMemFull, psiIoSome, psiIoFull,
    cpuPressure: cpu !== null && cpu >= 90, memoryPressure: ram !== null && ram >= 85, loadPressure: load !== null && load >= 1.5, swapPressure: swapIn !== null && swapIn >= 100,
    ioPressure: (iowait !== null && iowait >= 10) || (psiIoSome !== null && psiIoSome >= 5) || (psiIoFull !== null && psiIoFull >= 1),
    memoryStall: (psiMemSome !== null && psiMemSome >= 5) || (psiMemFull !== null && psiMemFull >= 1), cpuStall: (psiCpuSome !== null && psiCpuSome >= 10) || (psiCpuFull !== null && psiCpuFull >= 1),
  }
}

function rowEnhancedUsable(row = {}) {
  if (row.enhancedEvidenceUsable === false) return false
  if (row.enhancedEvidenceUsable === true) return true
  return metric(row.targetPssGb) !== null || metric(row.targetReadMiBps) !== null || metric(row.targetWriteMiBps) !== null || (row.wchanClass && row.wchanClass !== 'NONE')
}
...
export function incidentPatternV14(anchor = {}, rows = [], fallback = 'RESOURCE_CONTENTION') {
  ...
  const hasWchan = (name) => targetRows.some((row) => rowEnhancedUsable(row) && row.wchanClass === name)
  const hasD = targetRows.some((row) => Number(row.targetDState || 0) > 0)
  const highIoConsumer = targetRows.some((row) => rowEnhancedUsable(row) && (metric(row.targetReadMiBps) || 0) + (metric(row.targetWriteMiBps) || 0) >= 20)
  const pssSpike = targetRows.some((row) => rowEnhancedUsable(row) && (row.pssUplift?.score || 0) >= 0.5)
...
function pssCausalUnit(row = {}, anchor = {}) { if (!rowEnhancedUsable(row)) return 0; const host = hostSignal(anchor); if (!host.memoryPressure) return 0; return metric(row.targetPssGb) === null ? 0 : clamp01(row.pssUplift?.score || 0) }
function ioCausalUnit(row = {}, anchor = {}) { if (!rowEnhancedUsable(row)) return 0; const host = hostSignal(anchor); if (!host.ioPressure) return 0; return clamp01(((metric(row.targetReadMiBps) || 0) + (metric(row.targetWriteMiBps) || 0)) / 50) }
function enhancedBlockingUnit(row = {}, anchor = {}) {
  if (!rowEnhancedUsable(row)) return 0
  const host = hostSignal(anchor)
  const blockedClass = ['NFS', 'BLOCK_IO', 'MEMORY_RECLAIM', 'LOCK', 'NETWORK'].includes(row.wchanClass)
  if (!blockedClass) return 0
  return host.ioPressure || host.memoryStall || host.loadPressure || host.memoryPressure ? 1 : 0.5
}

export function refineWorkloadV14(row = {}, anchor = {}, capabilities = {}) {
  const taxonomy = errorTaxonomyForRow(row)
  const inferredEnhanced = rowEnhancedUsable(row)
  const telemetryMode = capabilities.mode || (inferredEnhanced ? 'PARTIAL' : 'LEGACY')
  const rowEnhanced = telemetryMode !== 'LEGACY' && inferredEnhanced
  ...
  let causalScore = Number(row.causalScore || 0); let victimScore = Number(row.victimScore || 0); let role = row.incidentRole || 'BACKGROUND'
  if (rowEnhanced) {
    causalScore += pss * 15; causalScore += io * 15; victimScore += blocked * 15
  ...
  if (!temporalGood) { causalScore *= 0.75; if (rowEnhanced) victimScore *= 0.85 }
  const relevanceScore = clamp(Math.max(Number(row.incidentScore || 0), causalScore, victimScore * 0.85))
  return {
    ...row, causalScore: Math.round(clamp(causalScore)), victimScore: Math.round(clamp(victimScore)), incidentScore: Math.round(relevanceScore), resourceScore: Math.round(relevanceScore), incidentRole: role, errorTaxonomy: taxonomy,
    telemetrySignalsV14: { pssCausal: Math.round(pss * 100), ioCausal: Math.round(io * 100), blockedVictim: Math.round(blocked * 100), errorCausalSupport: Math.round(errorSupport * 100) },
  }
}
...
export function verdictV14(rows = [], baseVerdict = {}, anchor = {}, capabilities = {}) {
  ...
  const top = ordered[0] || null; const second = ordered[1] || null
  const margin = top ? Number(top.causalScore || 0) - Number(second?.causalScore || 0) : 0
  const localGrade = top?.localConfidence?.grade || 'LOW'; const victimDominant = !!top && Number(top.victimScore || 0) > Number(top.causalScore || 0)
  ...
  const threshold = capabilities.mode === 'ENHANCED' && rowEnhancedUsable(top || {}) ? 65 : 70
  ...
  return {
    ...baseVerdict, status: singleSupported ? 'SINGLE_CULPRIT_SUPPORTED' : 'NO_SINGLE_CULPRIT', pattern, patternVersion: 'v2.1', topWorkload: top?.workload || '', topHost: top?.host || '', topCausalScore: Number(top?.causalScore || 0), topVictimScore: Number(top?.victimScore || 0), topLocalConfidence: localGrade, topRole: top?.incidentRole || '', margin, telemetryMode: capabilities.mode || 'LEGACY', reasons,
    interpretation: singleSupported ? 'One workload has a clear causal lead with high-quality incident evidence.' : 'No dominant initiating workload is confirmed by the available evidence.',
  }
}

export const __test = { hostSignal, pssCausalUnit, ioCausalUnit, enhancedBlockingUnit, timingForError, rowEnhancedUsable }
