...
function SnapshotStrip({ collection, incidentKey }) {
  ...
  return <section className="logV2Panel logV2SnapshotPanel">
    <div className="logV2PanelHead"><div><h2>Selected Collection · {range}{incident ? ' · RESOURCE INCIDENT TARGET' : ''}</h2><p>Each card retains its actual host timestamp. Cross-host timing confidence is {skewGrade} with {skew} min capture skew; ranking stays anchored to the automatic resource incident.</p></div><Status value={collection.severity} /></div>
...
function HostPeakSummary({ rca, selectedHost, onSelectHost }) {
  ...
  return <section className="logV2Panel">
    <div className="logV2PanelHead"><div><h2>Application Server Resource Peak Summary</h2><p>Instantaneous pressure prefers Load1/vCPU; Load15 remains context only. Sustained requires duration, while two points are labeled a short burst.</p></div></div>
...
function WorkloadDetail({ item }) {
  if (!item) return <section className="logV2Panel"><div className="logV2Empty">Select an observed workload from the role-aware incident ranking.</div></section>
  ...
  const breakdown = item.scoreBreakdownV4 || {}
  return <section className="logV2Panel">
    <div className="logV2PanelHead"><div><span className="logV2Eyebrow">SELECTED ROLE-AWARE INCIDENT WORKLOAD</span><h2>{item.workload}</h2><p>{item.host} · {item.program} · target {shortTime(item.targetTime)} · role {item.incidentRole} · confidence {item.incidentConfidence}</p></div><div className="logV2DetailBadges"><span className="logV2ScoreBadge">Causal {item.causalScore}/100</span><span className="logV2ScoreBadge">Relevance {item.incidentScore}/100</span><span className="logV2ScoreBadge">Victim {item.victimScore}/100</span><span className={`logV2ErrorBadge ${errorTone(item.errorState)}`}>{item.errorState}</span></div></div>
    <div className="logV2DetailGrid">
      <div className="logV2DetailFacts"><dl>
        <div><dt>Incident role</dt><dd>{item.incidentRole}</dd></div><div><dt>Evidence confidence</dt><dd>{item.incidentConfidence} · {item.confidence?.score ?? 0}/100</dd></div>
        <div><dt>Target evidence</dt><dd>{item.targetEvidence}{item.targetDeltaCollections ? ` Δ${item.targetDeltaCollections > 0 ? '+' : ''}${item.targetDeltaCollections}` : ''}</dd></div><div><dt>Collection skew</dt><dd>{item.confidence?.skewMinutes ?? 0} min · {item.confidence?.skewGrade || '—'}</dd></div>
        ...
        <div><dt>CPU contribution estimate</dt><dd>{metricText(item.cpuContributionPct, 1, '%')}</dd></div><div><dt>Max PID RSS / used RAM indicator</dt><dd>{metricText(item.maxPidRssUsedRamIndicatorPct, 1, '%')}</dd></div>
        ...
        <div><dt>CPU uplift vs baseline</dt><dd>{signed(item.cpuUplift?.delta, 1, '%')} · robust z {robustZText(item.cpuUplift?.z)}</dd></div><div><dt>RSS uplift vs baseline</dt><dd>{signed(item.rssUplift?.delta, 2, ' GB')} · robust z {robustZText(item.rssUplift?.z)}</dd></div>
        <div><dt>Baseline observations</dt><dd>{item.confidence?.baselineCount ?? 0}</dd></div><div><dt>Metric coverage</dt><dd>{hasMetric(item.confidence?.coveragePct) ? `${fmt(item.confidence.coveragePct, 0)}%` : '—'}</dd></div>
        ...
        <div className="wide"><dt>Role signals</dt><dd>consumer {item.roleSignals?.consumer ?? 0} · blocked/victim {item.roleSignals?.blockedVictim ?? 0} · error-cause {item.roleSignals?.errorCause ?? 0}</dd></div>
        <div className="wide"><dt>Causal score components</dt><dd>consumer {fmt(breakdown.consumer, 1)} · CPU uplift {fmt(breakdown.cpuUplift, 1)} · RSS uplift {fmt(breakdown.rssUplift, 1)} · error {fmt(breakdown.error, 1)} · temporal {fmt(breakdown.temporal, 1)} · host pressure {fmt(breakdown.hostPressure, 1)} · blocked/victim signal {fmt(breakdown.blockedVictim, 1)}</dd></div>
    ...
    <div className="logV2Method"><b>Interpretation:</b> Causal Priority does not award persistence and does not treat absolute RSS as exclusive physical-memory consumption. Max PID RSS / used RAM is contextual only; RSS enters causal ranking mainly through uplift versus the workload's own baseline. D-state raises victim/blocking evidence, not automatic culprit evidence. Relevance, causal priority, victim likelihood, role, and evidence confidence are intentionally separated.</div>
  </section>
}
...
export default function ToolLogAutoRcaV3() {
  ...
  const [status, setStatus] = React.useState('Upload WP-SCOUT / Daily Check logs. The v3.3 role-aware layer separates causal priority from blocked-victim evidence and reports incident confidence.')
  ...
  const rankResources = React.useCallback(async (nextAnalysis, nextRca) => {
    ...
    const ranked = await rankResourceConsumersV4(nextAnalysis?.processes || [], nextRca)
    setResourceRows(ranked.rows)
    setSelectedResource(ranked.rows[0] || null)
    setResourceEngine(ranked.engine)
    setDiagnostics({ parity: ranked.parity || { status: 'NOT_RUN' }, crossHostConfidence: ranked.crossHostConfidence || { grade: '—', skewMinutes: 0 }, engineReason: ranked.engineReason || '' })
  ...
  const upload = React.useCallback(async (list) => {
    setBusy(true)
    setStatus('Parsing evidence → schema validation → resource incident → role-aware causal/victim scoring → confidence…')
    try {
      ...
      setResourceRows([]); setSelectedResource(null)
  ...
  return <section className="logV2Shell"><div className="logV2Inner">
    <header className="logV2Header"><div><span className="logV2Eyebrow">DETERMINISTIC · ROLE-AWARE INCIDENT RCA</span><h1>LOG Analysis</h1><p>The engine anchors to the automatic resource incident, separates causal consumer evidence from blocked-victim evidence, treats RSS conservatively, grades evidence confidence, and reports DuckDB fallback/parity diagnostics.</p></div><label className="logV2Upload"><input type="file" multiple accept=".zip,.log,.txt,.csv" onChange={(event) => upload(event.target.files)} />{busy ? 'Analyzing…' : 'Upload Logs'}</label></header>
    <div className={`logV2StatusBar ${rca ? 'ready' : ''}`}>{status}</div>

    {!rca ? <div className="logV2EmptyState"><b>Role-aware investigation flow</b><span>Upload → validate → logical collections → landscape incident → workload aggregation → baseline uplift → role classification → causal/victim scores → confidence</span><p>Persistence is evidence confidence rather than causal bonus. D-state may indicate a blocked victim. Plain RSS is never treated as exclusive physical memory.</p></div> : <>
      <section className="logV2Stats">
        ...
        <Stat label="Incident target" value={`R ${shortTime(rca.resourceLandscapePeak?.timeLabel)}`} meta={`cross-host confidence ${diagnostics.crossHostConfidence?.grade || '—'} · skew ${diagnostics.crossHostConfidence?.skewMinutes ?? 0}m`} tone={(rca.resourceLandscapePeak?.resourceCrit || 0) > 0 ? 'critical' : ''} />
        <Stat label="Resource engine" value={ranking ? 'Ranking…' : resourceEngine} meta={`${mappingMeta} · parity ${diagnostics.parity?.status || 'NOT_RUN'}`} />
      </section>

      <section className="logV2Panel">
        <div className="logV2PanelHead"><div><h2>APP1–APP5 Logical Collection Timeline</h2><p>Load1/vCPU is used for instantaneous pressure. Clicking the timeline inspects another collection, while ranking remains anchored to the automatic resource incident.</p></div><div className="logV2MetricTabs">{Object.entries(LOG_V2_METRICS).map(([key, item]) => <button key={key} type="button" className={metric === key ? 'active' : ''} onClick={() => setMetric(key)}>{item.label}</button>)}</div></div>
      ...
      <section className="logV2Panel">
        <div className="logV2PanelHead"><div><h2>Observed Workloads · Role-Aware Causal Ranking</h2><p>Primary sort is Causal Priority. Relevance and Victim scores remain visible so a blocked process can be highly incident-related without being labeled the root cause.</p></div><div className="logV2Legend"><span><i className="resource" /> causal priority</span><span><i className="peak" /> evidence confidence</span><span><i className="error" /> role/error timing</span></div></div>
        {ranking ? <div className="logV2Empty"><b>Aggregating role-aware workload evidence…</b><br />Parsed {analysis?.processes?.length || 0} validated process rows.</div> : <VirtualResourceTable rows={resourceRows} selectedKey={selectedResource?.key || ''} onSelect={setSelectedResource} />}
      </section>
