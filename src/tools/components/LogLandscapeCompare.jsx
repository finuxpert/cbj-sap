...
export default function LogLandscapeCompare({ view, metric = 'memoryPct', onMetricChange, focusTime = '', onFocusTime, onHostDrilldown }) {
  ...
  const times = (view?.snapshots || []).map((row) => row.timeLabel)
  const rows = React.useMemo(() => {
    const map = new Map(times.map((timeLabel) => [timeLabel, { timeLabel }]))
    ;(view?.landscapeTelemetry || []).forEach((row) => {
      if (!map.has(row.timeLabel)) return
      map.get(row.timeLabel)[row.host] = Number.isFinite(Number(row[metric])) ? Number(row[metric]) : null
    })
    return Array.from(map.values())
  }, [view?.landscapeTelemetry, metric, times.join('|')])
  ...
  return <section className="rca26Panel rca26LandscapePanel">
    ...
    <div className="rca26PressureMatrix" data-pdf-ignore="true">
      <div className="rca26PressureRow header" style={{ '--rca-matrix-cols': Math.max(times.length, 1) }}><b>Host</b>{times.map((time) => <span key={time}>{time}</span>)}</div>
      {hosts.map((host) => <div className="rca26PressureRow" style={{ '--rca-matrix-cols': Math.max(times.length, 1) }} key={host}><button type="button" className="hostLabel" onClick={() => onHostDrilldown?.(host, focusTime || '')}>{host}</button>{times.map((time) => { const row = matrix.get(`${host}|${time}`); const severity = String(row?.severity || 'NONE').toLowerCase(); return <button type="button" key={`${host}-${time}`} className={`pressureCell ${severity}`} title={`${host} · ${time} · ${row?.severity || 'No sample'}`} data-active={focusTime === time} onClick={() => onHostDrilldown?.(host, time)}>{row ? row.severity.slice(0, 1) : '—'}</button> })}</div>)}
    </div>
  </section>
}
