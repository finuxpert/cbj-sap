...
export default function ToolLogAnalysis2026() {
  ...
  const reportTopTimes = reportTopJob ? expectedWorkloadTimes(analysis?.telemetry || [], reportTopJob.host, incident.start, incident.end) : []
  ...
  const recoveryText = view?.analytics?.windows?.after?.count ? `${view.analytics.windows.after.count} post-incident samples` : 'No post-incident data'
  ...
  return <section className="rca26Shell"><div className="rca26Inner" ref={reportRef} data-report-kind="log">
    ...
    <section className="rca26Panel rca26Deferred rca26IncidentErrors"><div className="rca26PanelHead"><div><h2>{jobScope === 'full' ? 'Analysis Window Errors' : 'Incident Errors'}</h2><p>Deduplicated across application servers by snapshot, WP and PID.</p></div></div><RcaDataTable rows={currentErrors} columns={errorColumns} searchPlaceholder="Search error code…" pageSize={50} defaultSort={{ key: 'affectedJobs', dir: 'desc' }} rowKey={(row) => row.errorCode} /></section>
