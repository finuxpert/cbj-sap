...
export default function ToolLogAnalysis2026() {
  ...
  return <section className="rca26Shell"><div className="rca26Inner" ref={reportRef} data-report-kind="log">
    ...
    <section className="rca26ControlBar">
      ...
      <label className="rca26Control rca26IncidentControl"><span>Host Incident</span><select value={incidentFocus} onChange={(event) => { const value = event.target.value; setIncidentFocus(value); setFocusTime(''); setJobScope(value === 'all' ? 'full' : 'incident'); clearSelection() }}><option value="all">All Evidence</option><option value="latest">Latest Host Incident</option><option value="peak">Peak Host Incident</option>{episodes.map((episode) => <option key={episode.key} value={episode.key}>{episode.label} · {compactTime(episode.start)} → {compactTime(episode.end)} · {episode.severity}</option>)}</select></label>
    ...
    <section className="rca26MetricStrip">
      <Metric label="Evidence Window" value={`${compactTime(evidenceWindow.start || '—')} → ${compactTime(evidenceWindow.end || '—')}`} meta={`${evidenceWindow.count || 0} snapshots · full uploaded scope`} />
      ...
      <Metric label="Host Incident Episodes" value={episodes.length} meta={`${episodeCounts.CRIT || 0} CRIT · ${episodeCounts.WARN || 0} WARN`} tone={(episodeCounts.CRIT || 0) > 0 ? 'critical' : (episodeCounts.WARN || 0) > 0 ? 'warn' : 'good'} />
      <Metric label="Selected Host Incident" value={incident.isAggregate ? 'ALL' : severity} meta={incident.count ? incident.isAggregate ? `${episodes.length} host episodes · ${incident.count} flagged snapshots` : `${compactTime(incident.start)} → ${compactTime(incident.end)}` : 'No host incident episode'} tone={severity === 'CRIT' ? 'critical' : severity === 'WARN' ? 'warn' : 'good'} />
      <Metric label="Landscape Impact" value={landscapeSeverity} meta={`${landscapeCritCount} CRIT · ${landscapeWarnCount} WARN · ${landscapeOverview.length} sampled hosts`} tone={landscapeSeverity === 'CRIT' ? 'critical' : landscapeSeverity === 'WARN' ? 'warn' : 'good'} />
    ...
    <section className="rca26Panel rca26Deferred rca26IncidentErrors"><div className="rca26PanelHead"><div><h2>{jobScope === 'full' ? 'Evidence Window Errors' : incident.isAggregate ? 'All Host Incident Episode Errors' : 'Selected Host Incident Errors'}</h2><p>Deduplicated across application servers by snapshot, WP and PID.</p></div></div><RcaDataTable rows={currentErrors} columns={errorColumns} searchPlaceholder="Search error code…" pageSize={50} defaultSort={{ key: 'affectedJobs', dir: 'desc' }} rowKey={(row) => row.errorCode} /></section>
