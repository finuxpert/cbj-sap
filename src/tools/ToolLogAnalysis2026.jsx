...
export default function ToolLogAnalysis2026() {
  ...
  const jobsTitle = focusTime ? `Workloads at ${focusTime}` : jobScope === 'full' ? 'Workloads in Full Window' : incident.count ? `Workloads During Incident · ${incident.start} – ${incident.end}` : 'No Incident Workloads'
  ...
  return <section className="rca26Shell"><div className="rca26Inner" ref={reportRef}>
    ...
    <section className="rca26Panel rca26Deferred"><div className="rca26PanelHead"><div><h2>{jobScope === 'full' ? 'Analysis Window Errors' : 'Incident Errors'}</h2><p>Deduplicated by snapshot, WP and PID.</p></div></div><RcaDataTable rows={currentErrors} columns={errorColumns} searchPlaceholder="Search error code…" pageSize={50} defaultSort={{ key: 'uniqueProcesses', dir: 'desc' }} rowKey={(row) => row.errorCode} /></section>
  </div></section>
}
