...
export default function ToolLogAnalysis2026() {
  ...
  const jobsTitle = focusTime ? `Workloads at ${focusTime} · All Servers` : jobScope === 'full' ? 'Workloads in Evidence Window · All Servers' : incident.isAggregate ? `Workloads Across ${episodes.length} Incident Windows · All Servers` : incident.count ? `Workloads During Selected Incident · ${incident.start} – ${incident.end} · All Servers` : 'No Incident Workloads'
  ...
  return <section className="rca26Shell"><div className="rca26Inner" ref={reportRef} data-report-kind="log">
    ...
    <section className="rca26ControlBar">
      ...
      <label className="rca26Control rca26IncidentControl"><span>Incident Window</span><select value={incidentFocus} onChange={(event) => { const value = event.target.value; setIncidentFocus(value); setFocusTime(''); setJobScope(value === 'all' ? 'full' : 'incident'); clearSelection() }}><option value="all">All Evidence</option><option value="latest">Latest Incident Window</option><option value="peak">Peak Pressure Window</option>{episodes.map((episode) => <option key={episode.key} value={episode.key}>{episode.label} · {compactTime(episode.start)} → {compactTime(episode.end)} · {episode.severity}</option>)}</select></label>
      ...
      <div className={`rca26Completeness ${completeness.isRegular ? 'ok' : 'neutral'}`}><b>{completeness.received} evidence snapshots</b><span>{completenessText}</span><small>{gapText} · line breaks mean missing evidence, not downtime proof</small></div>
    </section>

    <section className="rca26MetricStrip">
      <Metric label="Evidence Window" value={`${compactTime(evidenceWindow.start || '—')} → ${compactTime(evidenceWindow.end || '—')}`} meta={`${evidenceWindow.count || 0} snapshots · ${evidenceGaps.count ? `${evidenceGaps.count} evidence gaps` : 'continuous evidence'}`} />
      ...
      <Metric label="Detected Incident Windows" value={episodes.length} meta={`${episodeCounts.CRIT || 0} CRIT · ${episodeCounts.WARN || 0} WARN`} tone={(episodeCounts.CRIT || 0) > 0 ? 'critical' : (episodeCounts.WARN || 0) > 0 ? 'warn' : 'good'} />
      <Metric label="Selected Incident Window" value={incident.isAggregate ? 'ALL' : severity} meta={incident.count ? incident.isAggregate ? `${episodes.length} incident windows · ${incident.count} flagged snapshots` : `${compactTime(incident.start)} → ${compactTime(incident.end)}` : 'No host incident window'} tone={severity === 'CRIT' ? 'critical' : severity === 'WARN' ? 'warn' : 'good'} />
      <Metric label="Landscape Concurrency" value={landscapeSeverity} meta={`${landscapeCritCount} CRIT · ${landscapeWarnCount} WARN · ${landscapeConcurrent.sampled || 0} sampled at ${compactTime(landscapeConcurrent.timeLabel || '—')}`} tone={landscapeSeverity === 'CRIT' ? 'critical' : landscapeSeverity === 'WARN' ? 'warn' : 'good'} />
    ...
    <section className="rca26Panel rca26Deferred rca26IncidentErrors"><div className="rca26PanelHead"><div><h2>{jobScope === 'full' ? 'Evidence Window Errors' : incident.isAggregate ? 'All Incident Window Errors' : 'Selected Incident Window Errors'}</h2><p>Deduplicated across application servers by snapshot, WP and PID.</p></div></div><RcaDataTable rows={currentErrors} columns={errorColumns} searchPlaceholder="Search error code…" pageSize={50} defaultSort={{ key: 'affectedJobs', dir: 'desc' }} rowKey={(row) => row.errorCode} /></section>
    ...
    <div className="rca26IncidentReportOnly" aria-hidden="true">
      ...
      <section className="rca26Panel"><div className="rca26PanelHead"><div><h2>RCA Findings</h2><p>Deterministic evidence summary; not an automatic root-cause claim.</p></div></div><div className="rca26ReportFindings"><div className="rca26ReportFinding"><span>Evidence Window</span><b>{evidenceWindow.start}–{evidenceWindow.end} · {evidenceWindow.count} snapshots</b></div><div className="rca26ReportFinding"><span>Incident Window Scope</span><b>{incident.isAggregate ? `${episodes.length} incident windows` : incident.count ? `${incident.start}–${incident.end}` : 'No host incident detected'}</b></div><div className="rca26ReportFinding"><span>Landscape Impact</span><b>{landscapeSeverity} · {landscapeCritCount} CRIT · {landscapeWarnCount} WARN</b></div><div className="rca26ReportFinding"><span>Primary Host</span><b>{analysis?.primaryHost || view?.host || '—'}</b></div><div className="rca26ReportFinding"><span>Top Workload</span><b>{reportTopJob ? `${reportTopJob.name} · ${f(reportTopJob.anomalyScore, 1)}/100` : 'No qualifying workload'}</b></div><div className="rca26ReportFinding"><span>Key Signals</span><b>{reportKeySignals}</b></div><div className="rca26ReportFinding"><span>Recovery</span><b>{recoveryText}</b></div><div className="rca26ReportFinding"><span>Evidence Scope</span><b>{reportHosts.length} CRIT server{reportHosts.length === 1 ? '' : 's'} · {reportJobs.length} reported workload{reportJobs.length === 1 ? '' : 's'} · {reportErrors.length} reported error{reportErrors.length === 1 ? '' : 's'}</b></div></div></section>
    </div>
  </div></section>
}
