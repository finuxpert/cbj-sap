...
const LOG_COLORS = { CRIT: '#dc2626', WARN: '#d97706', OK: '#16a34a', BASIS: '#2563eb', ABAP: '#9333ea', DB: '#0f766e', UNKNOWN: '#64748b' }
const GRAPH_COLORS = [LOG_COLORS.BASIS, LOG_COLORS.DB, LOG_COLORS.WARN, LOG_COLORS.ABAP, LOG_COLORS.CRIT]
...
function group(rows, key) {
  ...
  return Array.from(map.values()).map((item) => ({ ...item, examples: Array.from(item.examples).slice(0, 3), jobs: Array.from(item.jobs).filter((value) => value !== '?').slice(0, 5), programs: Array.from(item.programs).filter((value) => value !== '?').slice(0, 5), times: Array.from(item.times).slice(0, 10), files: Array.from(item.files).slice(0, 5), sources: Array.from(item.sources).slice(0, 5) })).sort((a, b) => b.critHits - a.critHits || b.hits - a.hits)
}
...
function chartBase(extra = {}) {
  return { backgroundColor: 'transparent', color: GRAPH_COLORS, tooltip: { trigger: 'axis', backgroundColor: '#111827', borderWidth: 0, textStyle: { color: '#fff', fontSize: 12 } }, grid: { left: 126, right: 24, top: 18, bottom: 30, containLabel: true }, ...extra }
}
...
function ErrorRankingChart({ rows = [] }) {
  ...
  const option = chartBase({
    ...
    yAxis: { type: 'category', data: data.map((row) => displayLabel(row.name, 22)), axisLabel: { color: '#1e293b', fontSize: 11, fontWeight: 700 }, axisTick: { show: false }, axisLine: { show: false } },
    series: [{ type: 'bar', data: data.map((row) => ({ value: row.hits, row, itemStyle: { color: row.critHits ? LOG_COLORS.CRIT : LOG_COLORS.BASIS, borderRadius: [0, 7, 7, 0] } })), barWidth: 15, label: { show: true, position: 'right', color: '#334155', fontSize: 11, formatter: ({ data: item }) => `${item.row.hits} / C${item.row.critHits || 0}` } }],
  })
  return <section className="rcaFinalCard rcaFinalChartCard"><div className="rcaFinalPanelTitle"><h2>Top Error Code / Error Ranking</h2><span>Hits / CRIT</span></div>{data.length ? <ReactECharts option={option} style={{ height: 284, width: '100%' }} notMerge lazyUpdate /> : <p>No classified error rows.</p>}</section>
}

function ErrorTrendChart({ data = [] }) {
  const items = data.slice(-18)
  const option = chartBase({
    grid: { left: 44, right: 20, top: 22, bottom: 38, containLabel: true },
    legend: { bottom: 0, textStyle: { color: '#475569', fontSize: 11 } },
    xAxis: { type: 'category', data: items.map((item) => item.time), axisLabel: { color: '#64748b', fontSize: 11 }, axisLine: { lineStyle: { color: '#cbd5e1' } } },
    yAxis: { type: 'value', axisLabel: { color: '#64748b', fontSize: 11 }, splitLine: { lineStyle: { color: '#e2e8f0', type: 'dashed' } } },
    series: [{ name: 'Hits', type: 'line', smooth: true, symbolSize: 5, data: items.map((item) => item.hits), areaStyle: { opacity: 0.12 } }, { name: 'CRIT', type: 'line', smooth: true, symbolSize: 5, data: items.map((item) => item.crit), itemStyle: { color: LOG_COLORS.CRIT }, lineStyle: { color: LOG_COLORS.CRIT } }],
  })
  return <section className="rcaFinalCard rcaFinalChartCard"><div className="rcaFinalPanelTitle"><h2>Error Trend</h2><span>Time window</span></div>{items.length ? <ReactECharts option={option} style={{ height: 238, width: '100%' }} notMerge lazyUpdate /> : <p>No timeline data.</p>}</section>
}
