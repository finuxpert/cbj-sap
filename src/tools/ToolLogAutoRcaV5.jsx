...
function IncidentSummary({ verdict, capabilities }) {
  ...
  return <section className="logV2Panel logV141Summary">
    ...
    <div className="logV141SummaryGrid">
      ...
      <div><span>Pattern</span><strong>{humanize(verdict.pattern || 'RESOURCE_CONTENTION')}</strong><small>host-level incident</small></div>
      ...
      <div><span>Evidence</span><strong>{verdict.topLocalConfidence || 'LOW'}</strong><small>local workload evidence</small></div>
...
function SnapshotStrip({ collection, incidentKey, capabilities }) {
  ...
  return <section className="logV2Panel logV2SnapshotPanel">
    <div className="logV2PanelHead"><div><h2>{incident ? 'Incident Collection' : 'Selected Collection'} · {range}</h2><p>Cross-host skew {skew}m · telemetry {capabilities?.mode || 'LEGACY'}</p></div><Status value={collection.severity} /></div>
    <div className="logV2SnapshotGrid">{collection.rows.map((row) => <article key={row.host}><div><b>{row.host}</b><Status value={row.severity} /></div><small>{shortTime(row.timeLabel || row.snapshot)} · resource {row.resourceSeverity}</small><dl>
  ...
  return <section className="logV2Panel">
    ...
    <div className="logV2HostTableWrap"><table className="logV2HostTable"><thead><tr><th>Host</th><th>Operational</th><th>Resource</th><th>Peak</th><th>Time</th><th>Run</th><th>Max CPU</th><th>Max RAM</th><th>Max Load1</th><th>Max Swap</th><th>WP Critical</th></tr></thead><tbody>
      {rca.hostPeaks.map((item) => <tr key={item.host} className={selectedHost === item.host ? 'active' : ''} onClick={() => onSelectHost?.(item.host)}>
        ...
        <td>{item.sustained?.sustainedClass || 'NONE'} · {item.sustained?.sustainedScore || 0}</td><td>{metricText(item.metrics?.cpu?.value, 1, '%')}</td><td>{metricText(item.metrics?.ram?.value, 1, '%')}</td><td>{metricText(item.metrics?.load?.value, 2)}</td><td>{metricText(item.metrics?.swapIn?.value, 0, ' p/s')}</td><td>{metricText(item.metrics?.wpCritical?.value)}</td>
      </tr>)}
    </tbody></table></div>
  </section>
}
  ...
  return <section className="logV2Panel">
    ...
    <div className="logV2DetailGrid">
      <div className="logV2DetailFacts"><dl>
        <div><dt>Role</dt><dd>{humanize(item.incidentRole)}</dd></div><div><dt>Target</dt><dd>{item.targetEvidence} · {signed(item.targetDeltaMinutes, 0, 'm')}</dd></div>
        <div><dt>Evidence</dt><dd>{item.localConfidence?.grade || '—'} · {item.localConfidence?.score ?? '—'}/100</dd></div><div><dt>Cross-host</dt><dd>{item.landscapeConfidence?.grade || '—'} · {item.landscapeConfidence?.score ?? '—'}/100</dd></div>
        ...
        <div><dt>Blocking WCHAN</dt><dd>{item.wchanClass || 'NONE'} · {item.targetWchan || '—'}{item.wchanScope === 'D_STATE' ? ' · D-state first' : ''}</dd></div><div><dt>Enhanced timing</dt><dd>{item.enhancedEvidenceQuality || 'UNAVAILABLE'}{hasMetric(item.enhancedEvidenceDeltaMinutes) ? ` · ${item.enhancedEvidenceDeltaMinutes}m` : ''}</dd></div>
        ...
        <div><dt>Error class</dt><dd>{taxonomy.strongest?.category || 'NONE'}</dd></div><div><dt>Error direction</dt><dd>{taxonomy.direction || 'CONTEXT'}</dd></div>
...
function AnalyticsDiagnostics({ diagnostics, capabilities, mapping, verdict }) {
  const parity = diagnostics?.parity || {}
  return <details className="logV2SourceAudit"><summary>Diagnostics</summary><div><table><tbody>
    <tr><th>RCA engine</th><td>{diagnostics?.engineDiagnostics?.rcaEngine || 'RCA v3.6.1'}</td></tr>
  ...
  return <section className="logV2Shell"><div className="logV2Inner">
    ...
    {!rca ? <div className="logV2EmptyState"><b>Upload evidence</b><p>WP-SCOUT / Daily Check logs. Enhanced Linux telemetry is optional.</p></div> : <>
      <section className="logV2Stats">
        ...
        <Stat label="Verdict" value={ranking ? 'Ranking…' : verdict?.status === 'SINGLE_CULPRIT_SUPPORTED' ? 'DOMINANT WORKLOAD' : 'NO DOMINANT WORKLOAD'} meta={ranking ? 'working' : humanize(verdict?.pattern || '—')} />
      ...
      <section className="logV2Panel">
        <div className="logV2PanelHead"><div><h2>Workload Ranking</h2><p>Priority = incident causal evidence. Open a row for full forensic detail.</p></div></div>
      ...
      <AnalyticsDiagnostics diagnostics={diagnostics} capabilities={capabilities} mapping={mapping} verdict={verdict} />
