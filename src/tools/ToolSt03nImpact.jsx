...
function findHeaderIndex(rows = []) {
  ...
  rows.slice(0, 80).forEach((row, idx) => {
    ...
    let s = 0
    if (/transaction|report|program|time interval|task type|tcode|dialog|user/.test(line)) s += 4
    if (/response|database|db time|dialog steps|cpu|wait|average|total|elapsed|duration/.test(line)) s += 4
    if (/name|client|object|count|calls/.test(line)) s += 1
...
function summarize(kind, objects, fileName) {
  ...
  const rows = objects.map((o, idx) => {
    ...
    const responseMs = num(o[cResp], 0)
    const dbMs = num(o[cDb], 0)
    const waitMs = num(o[cWait], 0)
    const steps = num(o[cSteps], 0)
...
export default function ToolSt03nImpact() {
    ...
    try {
      ...
      for (const req of REQUIRED) {
        ...
        for (const file of group) {
          const objects = rowsToObjects(await readMatrix(file))
      ...
      const parsedFiles = Object.values(result.detected).flat().length
      result.impact = top ? 'Detected' : 'Not confirmed'
      result.dominant = dbRows >= waitRows && dbRows >= responseRows ? 'DB-heavy' : waitRows >= responseRows ? 'Wait-heavy' : top ? 'Response-heavy' : 'Unknown'
      result.correlation = top && session ? 'Partial correlation: compare this evidence with RCA window.' : top ? 'Standalone ST03N impact only' : 'Weak: no readable ST03N metrics'
      ...
      result.completeness = Math.round((parsedFiles / REQUIRED.length) * 100)
      setAnalysis(result)
  ...
  const top = topRows[0]
  return <section className="evidenceToolShell refinedTool"><header className="evidenceHero compactEvidenceHero"><div><span>ST03N Impact Analyzer</span><h1>Workload impact drilldown.</h1><p>Use this page after the main RCA to confirm whether slowdown is visible in transaction/workload evidence.</p></div><label className="evidenceUpload"><input type="file" multiple accept=".zip,.xlsx,.xls,.csv" onChange={(e) => onFiles(e.target.files)} />Upload ST03N Pack</label></header>{session && <section className="sessionBanner"><b>Latest RCA session</b><span>{session.sid} • {session.host} • {session.window?.start} - {session.window?.end}</span><small>{session.summary}</small></section>}<section className="decisionBoard"><DecisionCard label="Impact Verdict" value={analysis?.impact || 'Pending'} hint={analysis?.summary || status} tone={analysis?.impact === 'Detected' ? 'good' : ''} /><DecisionCard label="Dominant Component" value={analysis?.dominant || 'Unknown'} hint="Response / DB / Wait based on parsed ST03N columns" tone="blue" /><DecisionCard label="Correlation" value={analysis?.correlation ? analysis.correlation.split(':')[0] : 'Pending'} hint={analysis?.correlation || 'Upload ST03N pack to validate workload impact'} /><DecisionCard label="Completeness" value={`${analysis?.completeness || 0}%`} hint="Detected ST03N evidence pack coverage" /></section><div className="evidenceGrid"><section className="evidencePanel"><h2>ST03N Parse Status</h2><div className="statusList">{REQUIRED.map((req) => <div key={req.key} className={detected[req.key]?.length ? 'ok' : 'missing'}><b>{req.label}</b><span>{detected[req.key]?.[0]?.name || analysis?.parseStatus?.find((x) => x.key === req.key)?.message || 'missing'}</span></div>)}</div></section><section className="evidencePanel"><h2>Interpretation</h2>{top ? <><p><b>{top.label}</b> is currently the highest ST03N workload signal.</p><div className="confidenceRows"><span>Score<b>{top.score}/100</b></span><span>Component<b>{top.component}</b></span><span>Readable Rows<b>{analysis?.rows?.length || 0}</b></span></div></> : <p>{status}</p>}</section></div>{analysis && <div className="evidenceGrid wide"><section className="evidencePanel chartPanel"><h2>Top ST03N Evidence</h2><ResponsiveContainer width="100%" height={320}><BarChart data={topRows.map((r) => ({ name: r.label.slice(0, 18), score: r.score, response: Math.round(r.responseMs), db: Math.round(r.dbMs), wait: Math.round(r.waitMs) }))}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" /><YAxis domain={[0, 100]} /><Tooltip /><Legend /><Bar dataKey="score" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></section><section className="evidencePanel"><h2>Top Offenders</h2><div className="evidenceList">{topRows.map((r) => <div key={`${r.kind}-${r.fileName}-${r.label}`}><b>{r.label}</b><span>{r.kind} · {r.component} · score {r.score}/100</span><small>Response {fmt(r.responseMs, 0)}ms · DB {fmt(r.dbMs, 0)}ms · Wait {fmt(r.waitMs, 0)}ms · Steps {fmt(r.steps, 0)}</small></div>)}</div></section></div>}</section>
}
